...
class Scene:
    ...
    def ray_trace(self, ray, depth):
        ...
        if closest_object != 0:
            ...
            color = closest_object.material.color * self.shade(
                intersect, normal, direction
            )
            ...
            if depth > 0 and reflectivity > 0:
                ...
                reflection_color = self.ray_trace(
                    Ray(intersect, reflection_direction), depth - 1
                )
    ...
    def calculate_intersection(self, ray):
        ...
        for object in self.objects:
            ...
            if self.constrain(t0, closest_t):
                closest_t, closest_object = t0, object
            if self.constrain(t1, closest_t):
                closest_t, closest_object = t1, object
            ...
            for x in range(self.camera.canvas_width):
                for y in range(self.camera.canvas_height):
                    ...
                    color = self.ray_trace(
                        Ray(origin, (origin - self.camera.position).norm()), depth
                    )
